...
function round_robin_group_matches_complete(array $matchRecords): bool
{
    $hasRealMatch = false;
    ...
    foreach ($matchRecords as $record) {
        ...
        $player1 = isset($record['player1_user_id']) ? (int)$record['player1_user_id'] : null;
        $player2 = isset($record['player2_user_id']) ? (int)$record['player2_user_id'] : null;

        if ($player1 && $player2) {
            $hasRealMatch = true;
            if (empty($record['winner_user_id'])) {
                return false;
            }
        }
    }
...
function tournament_matches_complete(array $matches, string $type): bool
{
    foreach ($matches as $match) {
        $player1 = isset($match['player1_user_id']) ? (int)$match['player1_user_id'] : 0;
        $player2 = isset($match['player2_user_id']) ? (int)$match['player2_user_id'] : 0;
    ...
    if ($type === 'round-robin') {
        return round_robin_group_matches_complete($matches);
    }

    return true;
}

function compute_round_robin_standings(array $players, array $matchRecords): array
{
    $stats = [];

    foreach ($players as $participant) {
        $playerId = isset($participant['user_id']) ? (int)$participant['user_id'] : null;
        if (!$playerId) {
            continue;
        }
        ...
        $stats[$playerId] = [
            'player_id' => $playerId,
            'name' => $participant['username'] ?? ('Player ' . $playerId),
            'wins' => 0,
            'losses' => 0,
            'matches_played' => 0,
        ];
    ...
    foreach ($matchRecords as $record) {
        ...
        $player1 = isset($record['player1_user_id']) ? (int)$record['player1_user_id'] : null;
        $player2 = isset($record['player2_user_id']) ? (int)$record['player2_user_id'] : null;
        $winner = isset($record['winner_user_id']) ? (int)$record['winner_user_id'] : null;

        if ($player1 && !isset($stats[$player1])) {
            $stats[$player1] = [
                'player_id' => $player1,
                'name' => $record['player1_name'] ?? ('Player ' . $player1),
                'wins' => 0,
                'losses' => 0,
                'matches_played' => 0,
            ];
        ...
        if ($player2 && !isset($stats[$player2])) {
            $stats[$player2] = [
                'player_id' => $player2,
                'name' => $record['player2_name'] ?? ('Player ' . $player2),
                'wins' => 0,
                'losses' => 0,
                'matches_played' => 0,
            ];
        ...
        if ($player1) {
            $stats[$player1]['matches_played']++;
        }
        if ($player2) {
            $stats[$player2]['matches_played']++;
        }
...
function round_robin_leaders(array $standings): array
{
    if (empty($standings)) {
        return [];
    }

    $maxWins = max(array_column($standings, 'wins'));

    return array_values(array_filter($standings, static function (array $entry) use ($maxWins): bool {
        return isset($entry['wins']) && (int)$entry['wins'] === (int)$maxWins;
    }));
}

function delete_round_robin_finals(int $tournamentId): void
{
    db()->prepare("DELETE FROM tournament_matches WHERE tournament_id = :tid AND stage = 'finals'")
        ->execute([':tid' => $tournamentId]);
}

function ensure_round_robin_final_match(int $tournamentId, array $leaders): void
{
    if (count($leaders) < 2) {
        delete_round_robin_finals($tournamentId);
        return;
    }

    $player1 = isset($leaders[0]['player_id']) ? (int)$leaders[0]['player_id'] : null;
    $player2 = isset($leaders[1]['player_id']) ? (int)$leaders[1]['player_id'] : null;
    ...
    if ($existing) {
        $needsReset = ((int)($existing['player1_user_id'] ?? 0) !== $player1)
            || ((int)($existing['player2_user_id'] ?? 0) !== $player2);

        if ($needsReset) {
            $pdo->prepare("UPDATE tournament_matches SET player1_user_id = :p1, player2_user_id = :p2, score1 = NULL, score2 = NULL, winner_user_id = NULL, meta = :meta WHERE id = :id")
                ->execute([
                    ':p1' => $player1,
                    ':p2' => $player2,
                    ':meta' => $metaJson,
            ...
            $pdo->prepare('UPDATE tournament_matches SET meta = :meta WHERE id = :id')
                ->execute([
                    ':meta' => $metaJson,
                    ':id' => $existing['id'],
                ]);
    ...
    $pdo->prepare("INSERT INTO tournament_matches (tournament_id, stage, round, match_index, player1_user_id, player2_user_id, meta) VALUES (:tid, 'finals', 1, 1, :p1, :p2, :meta)")
        ->execute([
            ':tid' => $tournamentId,
            ':p1' => $player1,
            ':p2' => $player2,
...
function synchronize_round_robin_finals(int $tournamentId): void
{
    $players = tournament_players($tournamentId);
    $matches = tournament_matches($tournamentId);

    $groupMatches = array_values(array_filter($matches, static function (array $match): bool {
        return ($match['stage'] ?? '') === 'group';
    }));

    $finalMatches = array_values(array_filter($matches, static function (array $match): bool {
        return ($match['stage'] ?? '') === 'finals';
    }));

    $complete = round_robin_group_matches_complete($groupMatches);
    ...
    $standings = compute_round_robin_standings($players, $matches);
    $leaders = round_robin_leaders($standings);
    ...
    ensure_round_robin_final_match($tournamentId, $leaders);
}

function round_robin_pair_key(?int $player1, ?int $player2): ?string
{
...
function round_robin_consume_record(array &$pairMap, array $record): void
{
    if (empty($record['id'])) {
        return;
    }
    ...
    $player1 = isset($record['player1_user_id']) ? (int)$record['player1_user_id'] : null;
    $player2 = isset($record['player2_user_id']) ? (int)$record['player2_user_id'] : null;
    $key = round_robin_pair_key($player1, $player2);

    if ($key === null || empty($pairMap[$key])) {
        return;
    }

    foreach ($pairMap[$key] as $index => $candidate) {
        if (empty($candidate['id'])) {
            continue;
        }
        if ((int)$candidate['id'] === (int)$record['id']) {
            array_splice($pairMap[$key], $index, 1);
...
function round_robin_pull_pair_match(array &$pairMap, ?int $player1, ?int $player2): ?array
{
    $key = round_robin_pair_key($player1, $player2);
    if ($key === null || empty($pairMap[$key])) {
        return null;
    ...
    $record = array_shift($pairMap[$key]);
    if (empty($pairMap[$key])) {
        unset($pairMap[$key]);
    }
...
function seed_round_robin_teams(array $players): array
{
    $teams = [];
    $playerMap = [];
    $nextId = 1;
    ...
    return [$teams, $playerMap, $hasBye, $byeTeamId];
}

function generate_round_robin_pairings(array $players): array
{
    [$teams] = seed_round_robin_teams($players);
    ...
    for ($round = 0; $round < $roundCount; $round++) {
        for ($i = 0; $i < $half; $i++) {
            $homeTeam = $teams[$positions[$i]];
            $awayTeam = $teams[$positions[$teamCount - 1 - $i]];

            if (!empty($homeTeam['is_bye']) || !empty($awayTeam['is_bye'])) {
                continue;
            }

            $player1 = isset($homeTeam['player_id']) ? (int)$homeTeam['player_id'] : null;
            $player2 = isset($awayTeam['player_id']) ? (int)$awayTeam['player_id'] : null;
            ...
            $pairs[] = [
                'round' => $roundNumber,
                'match_number' => $roundCounters[$roundNumber],
...
function rebuild_round_robin_group_matches(int $tournamentId): bool
{
    $tournament = get_tournament($tournamentId);
    if (!$tournament || $tournament['type'] !== 'round-robin') {
        return false;
    }
    ...
    $expectedPairs = generate_round_robin_pairings($players);
    if (empty($expectedPairs)) {
        return false;
    }

    $pdo = db();
    $stmt = $pdo->prepare(
        "SELECT id, round, match_index, player1_user_id, player2_user_id, winner_user_id\n         FROM tournament_matches\n         WHERE tournament_id = :tid AND stage = 'group'"
    );
    $stmt->execute([':tid' => $tournamentId]);
    ...
    foreach ($existingMatches as $match) {
        $key = round_robin_pair_key(
            isset($match['player1_user_id']) ? (int)$match['player1_user_id'] : null,
            isset($match['player2_user_id']) ? (int)$match['player2_user_id'] : null
        );
        ...
        $currentWinner = isset($existingByPair[$key]['winner_user_id']) ? (int)$existingByPair[$key]['winner_user_id'] : null;
        $candidateWinner = isset($match['winner_user_id']) ? (int)$match['winner_user_id'] : null;

        if (!$currentWinner && $candidateWinner) {
            $existingByPair[$key] = $match;
        }
    }

    $expectedCount = count($expectedPairs);
    $existingCount = count($existingMatches);
    $needsRepair = $existingCount !== $expectedCount;

    if (!$needsRepair) {
        foreach ($expectedPairs as $pair) {
            $key = round_robin_pair_key($pair['player1_user_id'], $pair['player2_user_id']);
            if ($key === null) {
                continue;
            }
            if (!isset($existingByPair[$key])) {
    ...
    try {
        ...
        $delete = $pdo->prepare("DELETE FROM tournament_matches WHERE tournament_id = :tid AND stage = 'group'");
        $delete->execute([':tid' => $tournamentId]);

        $insert = $pdo->prepare(
            "INSERT INTO tournament_matches (tournament_id, stage, round, match_index, player1_user_id, player2_user_id, winner_user_id)\n             VALUES (:tid, 'group', :round, :index, :p1, :p2, :winner)"
        );

        foreach ($expectedPairs as $pair) {
            $player1 = (int)$pair['player1_user_id'];
            $player2 = (int)$pair['player2_user_id'];
            $winner = null;
            $key = round_robin_pair_key($player1, $player2);

            if ($key !== null && isset($existingByPair[$key])) {
                $candidate = $existingByPair[$key];
                $candidateWinner = isset($candidate['winner_user_id']) ? (int)$candidate['winner_user_id'] : null;
                if ($candidateWinner === $player1 || $candidateWinner === $player2) {
        ...
        error_log('Failed to rebuild round robin matches: ' . $e->getMessage());
        return false;
    }

    synchronize_round_robin_finals($tournamentId);
    return true;
}

function build_round_robin_groups(array $players, array $matchRecords = []): array
{
    [$teams, $playerMap, $hasBye, $byeTeamId] = seed_round_robin_teams($players);
    ...
    foreach ($matchRecords as $record) {
        ...
        $index = (int)($record['match_index'] ?? 0);
        if ($round <= 0 || $index <= 0) {
            continue;
        }
        if (!isset($roundMatchMap[$round])) {
            $roundMatchMap[$round] = [];
        }
        $roundMatchMap[$round][$index] = $record;

        $pairKey = round_robin_pair_key(
            isset($record['player1_user_id']) ? (int)$record['player1_user_id'] : null,
            isset($record['player2_user_id']) ? (int)$record['player2_user_id'] : null
        );
        if ($pairKey !== null) {
            if (!isset($pairMatchMap[$pairKey])) {
                $pairMatchMap[$pairKey] = [];
            }
            $pairMatchMap[$pairKey][] = $record;
        }
    }
    ...
    if ($teamCount > 1) {
        ...
        for ($round = 0; $round < $roundCount; $round++) {
            for ($i = 0; $i < $half; $i++) {
                ...
                $roundNumber = $round + 1;
                $roundCounters[$roundNumber] = ($roundCounters[$roundNumber] ?? 0) + 1;
                $matchNumber = $roundCounters[$roundNumber];
                $player1Id = $homeTeam['player_id'] ?? ($playerMap[$homeTeam['id']] ?? null);
                $player2Id = $awayTeam['player_id'] ?? ($playerMap[$awayTeam['id']] ?? null);
                $player1Name = $homeTeam['name'] ?? 'TBD';
                $player2Name = $awayTeam['name'] ?? 'TBD';

                $existing = $roundMatchMap[$roundNumber][$matchNumber] ?? null;
                if ($existing) {
                    round_robin_consume_record($pairMatchMap, $existing);
                } else {
                    $existing = round_robin_pull_pair_match($pairMatchMap, $player1Id, $player2Id);
                }

                if ($existing) {
                    $player1Id = $existing['player1_user_id'] ? (int)$existing['player1_user_id'] : $player1Id;
                    $player2Id = $existing['player2_user_id'] ? (int)$existing['player2_user_id'] : $player2Id;
                    if (!empty($existing['player1_name'])) {
                        $player1Name = (string)$existing['player1_name'];
                    }
                    if (!empty($existing['player2_name'])) {
                        $player2Name = (string)$existing['player2_name'];
                    }
                }

                $winnerId = $existing && $existing['winner_user_id'] ? (int)$existing['winner_user_id'] : null;
                $winnerSlot = null;
                if ($winnerId !== null) {
                    if ($player1Id && $winnerId === (int)$player1Id) {
                        $winnerSlot = 1;
                    } elseif ($player2Id && $winnerId === (int)$player2Id) {
                        $winnerSlot = 2;
                    }
                }

                $winnerName = $existing['winner_name'] ?? null;
                ...
                if ($player1Id || ($player1Name && strtoupper($player1Name) !== 'BYE')) {
                    $meta['player1'] = [
                        'id' => $player1Id !== null ? (int)$player1Id : null,
                        'name' => $player1Name,
                        'team_index' => $homeIndex,
                    ];
                }

                if ($player2Id || ($player2Name && strtoupper($player2Name) !== 'BYE')) {
                    $meta['player2'] = [
                        'id' => $player2Id !== null ? (int)$player2Id : null,
                        'name' => $player2Name,
    ...
    foreach ($teams as $index => $team) {
        if (!empty($team['player_id'])) {
            $playerIndexMap[(int)$team['player_id']] = $index;
        }
    }

    foreach ($finalMatchRecords as $record) {
        $roundNumber = isset($record['round']) ? (int)$record['round'] : 1;
        if ($roundNumber <= 0) {
            $roundNumber = 1;
        }

        $matchNumber = isset($record['match_index']) ? (int)$record['match_index'] : 1;
        if ($matchNumber <= 0) {
            $matchNumber = 1;
        }

        $player1Id = isset($record['player1_user_id']) ? (int)$record['player1_user_id'] : null;
        $player2Id = isset($record['player2_user_id']) ? (int)$record['player2_user_id'] : null;

        if ($player1Id === null || $player2Id === null) {
            continue;
        }

        if (!array_key_exists($player1Id, $playerIndexMap) || !array_key_exists($player2Id, $playerIndexMap)) {
            continue;
        }
        ...
        $meta = [
            'stage' => 'finals',
            'match_id' => isset($record['id']) ? (int)$record['id'] : null,
            'match_number' => $matchNumber,
            'round_number' => $roundNumber,
            'stage_label' => 'Finals',
            'player1' => [
    ...
    $standings = compute_round_robin_standings($players, $matchRecords);
    foreach ($standings as &$entry) {
        $playerId = $entry['player_id'] ?? null;
        if ($playerId && isset($playerIndexMap[$playerId])) {
            $entry['team_index'] = $playerIndexMap[$playerId];
        }
    }
    unset($entry);
    ...
    $groupComplete = round_robin_group_matches_complete($matchRecords);
    $meta = [
        'format' => 'round-robin',
        'rounds' => $roundCount,
        'has_bye' => $hasBye,
        ...
        'finals_required' => count($leaders) > 1,
    ];

    if ($groupComplete) {
        if ($meta['finals_required']) {
            foreach ($finalMatchRecords as $record) {
                if (!empty($record['winner_user_id'])) {
                    $meta['champion'] = [
                        'id' => (int)$record['winner_user_id'],
                        'name' => $record['winner_name'] ?? null,
                    ];
                    $meta['finals_required'] = false;
                    break;
                }
            }
        } elseif (count($leaders) === 1) {
            $meta['champion'] = [
                'id' => $leaders[0]['player_id'],
                'name' => $leaders[0]['name'],
            ];
        }
    }
...
function generate_bracket_structure(int $tournamentId): array
{
    ...
    if ($tournament['type'] === 'round-robin') {
        rebuild_round_robin_group_matches($tournamentId);
        $players = tournament_players($tournamentId);
        $matches = tournament_matches($tournamentId);
        return build_round_robin_groups($players, $matches);
    }
...
function build_double_elimination_bracket(int $tournamentId): array
{
    ...
    foreach ($layout['finals'] as $round => $roundMatches) {
        ...
        foreach ($roundMatches as $matchIndex => $node) {
            ...
            $matchMeta = $match ? decode_match_meta($match['meta'] ?? null) : [];
            $isConditional = !empty($node['if_necessary']) || !empty($matchMeta['if_necessary']);
            $isVisible = !$isConditional || !empty($matchMeta['visible']);
            if ($isConditional && !$isVisible) {
                continue;
            }
            $entry = $buildMatch($match, $node, $round - 1, $matchIndex - 1);
            if ($isConditional) {
                $entry[2]['if_necessary'] = true;
                if (!empty($matchMeta['visible'])) {
                    $entry[2]['visible'] = true;
                }
            }
            if (!empty($matchMeta['reset'])) {
                $entry[2]['reset'] = $matchMeta['reset'];
            }
            $roundData[] = $entry;
        }
        if (!empty($roundData)) {
...
function seed_double_elimination_matches(int $tournamentId, array $players): void
{
    ...
    foreach ($layout['finals'] as $round => $roundMatches) {
        foreach ($roundMatches as $matchIndex => $node) {
            ...
            if (!empty($node['reset'])) {
                $meta['reset'] = $node['reset'];
            }
            if (!empty($node['if_necessary'])) {
                $meta['if_necessary'] = true;
                $meta['visible'] = false;
            }
            $insert->execute([
                ':tid' => $tournamentId,
...
function record_match_result(int $tournamentId, int $matchId, ?int $winnerId): array
{
    ...
    $matches = tournament_matches($tournamentId);
    $championId = determine_tournament_champion($tournament, $structure, $matches);
    $type = $tournament['type'] ?? '';
    if ($championId !== null && tournament_matches_complete($matches, $type)) {
        if (($tournament['status'] ?? '') !== 'completed') {
            update_tournament_status($tournamentId, 'completed');
...
function propagate_winner_to_next_match(int $tournamentId, array $match, ?int $winnerId): void
{
    ...
    $resetMeta = [];
    if (!empty($meta['reset']) && is_array($meta['reset'])) {
        $resetMeta = $meta['reset'];
        $handled = true;
        $triggerSlot = isset($resetMeta['trigger_slot']) ? (int)$resetMeta['trigger_slot'] : 0;
        $winnerSlot = null;
        $player1Id = isset($match['player1_user_id']) ? (int)$match['player1_user_id'] : null;
        $player2Id = isset($match['player2_user_id']) ? (int)$match['player2_user_id'] : null;
        if ($winnerId) {
        ...
        $resetStage = $resetMeta['stage'] ?? ($meta['stage'] ?? 'finals');
        $resetRound = isset($resetMeta['round']) ? (int)$resetMeta['round'] : null;
        $resetMatchIndex = isset($resetMeta['match_index']) ? (int)$resetMeta['match_index'] : null;
        if ($resetStage && $resetRound && $resetMatchIndex) {
            $resetMatch = find_match_by_coordinates($tournamentId, $resetStage, $resetRound, $resetMatchIndex);
            if ($resetMatch) {
                $winnerDestination = $resetMeta['winner_destination'] ?? null;
                $loserDestination = $resetMeta['loser_destination'] ?? null;
                if (!$winnerDestination) {
                    $winnerDestination = [
                        'stage' => $resetStage,
                        'round' => $resetRound,
                        'match_index' => $resetMatchIndex,
                        'slot' => isset($resetMeta['winner_slot']) ? (int)$resetMeta['winner_slot'] : 2,
                    ];
                }
                if (!$loserDestination) {
                    $loserDestination = [
                        ...
                        'round' => $resetRound,
                        'match_index' => $resetMatchIndex,
                        'slot' => isset($resetMeta['loser_slot']) ? (int)$resetMeta['loser_slot'] : 1,
                    ];
                }

                if ($winnerId && $winnerSlot !== null && $triggerSlot > 0 && $winnerSlot === $triggerSlot) {
                    $loserId = null;
                    if ($player1Id && $player1Id !== $winnerId) {
                        $loserId = $player1Id;
                    } elseif ($player2Id && $player2Id !== $winnerId) {
                    ...
                    apply_match_destination($tournamentId, $winnerDestination, $winnerId);
                    apply_match_destination($tournamentId, $loserDestination, $loserId);

                    $targetMeta = decode_match_meta($resetMatch['meta'] ?? null);
                    $targetMeta['if_necessary'] = true;
                    $targetMeta['visible'] = true;
                    $targetMeta['stage'] = $targetMeta['stage'] ?? $resetStage;
                    db()->prepare('UPDATE tournament_matches SET meta = :meta WHERE id = :id')
                        ->execute([
                            ':meta' => json_encode($targetMeta),
                            ':id' => $resetMatch['id'],
                        ]);
                } else {
                    db()->prepare('UPDATE tournament_matches SET player1_user_id = NULL, player2_user_id = NULL, score1 = NULL, score2 = NULL, winner_user_id = NULL WHERE id = :id')
                        ->execute([':id' => $resetMatch['id']]);
                    $targetMeta = decode_match_meta($resetMatch['meta'] ?? null);
                    $targetMeta['if_necessary'] = true;
                    $targetMeta['visible'] = false;
                    $targetMeta['stage'] = $targetMeta['stage'] ?? $resetStage;
                    db()->prepare('UPDATE tournament_matches SET meta = :meta WHERE id = :id')
                        ->execute([
                            ':meta' => json_encode($targetMeta),
...
function clear_following_results(int $tournamentId, array $match, array &$visited = []): void
{
    ...
    if (!empty($meta['reset']) && is_array($meta['reset'])) {
        $reset = $meta['reset'];
        $stage = $reset['stage'] ?? ($meta['stage'] ?? null);
        $round = isset($reset['round']) ? (int)$reset['round'] : null;
        $matchIndex = isset($reset['match_index']) ? (int)$reset['match_index'] : null;
        if ($stage && $round && $matchIndex) {
            $target = find_match_by_coordinates($tournamentId, $stage, $round, $matchIndex);
            if ($target) {
                db()->prepare('UPDATE tournament_matches SET player1_user_id = NULL, player2_user_id = NULL, score1 = NULL, score2 = NULL, winner_user_id = NULL WHERE id = :id')
                ...
                $targetMeta['if_necessary'] = true;
                $targetMeta['visible'] = false;
                $targetMeta['stage'] = $targetMeta['stage'] ?? $stage;
                db()->prepare('UPDATE tournament_matches SET meta = :meta WHERE id = :id')
                    ->execute([
                        ':meta' => json_encode($targetMeta),
                        ':id' => $target['id'],
...
function determine_tournament_champion(array $tournament, ?array $structure = null, ?array $matches = null): ?int
{
    ...
    if ($type === 'round-robin') {
        ...
        $groupComplete = !empty($structure['meta']['group_complete']);
        if (!$groupComplete) {
            return null;
        }
    }
